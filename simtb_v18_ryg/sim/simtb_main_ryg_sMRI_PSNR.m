function simtb_main_ryg_sMRI_PSNR(sP,PSNR)
...
%     end
 if sP.D_noise_FLAG
    for i=1:size(D,1) %%%%%%%%%%%%%%%%%%%%% ryg, adding noise to each scan since here we treat 
                       %%%%%%%%%%%%%%%%%%%%% each scan as a sMRI from one single participant  
        mask = simtb_createmask(sP);
        mask = find(mask == 1);
%         signal_std(i,1) = std(D(i,mask));
        signal_std(i,1) = mean(D(i,mask));
...
%     end
    if sP.D_noise_FLAG %%%%%%%%%%%%%%%%%%%%% ryg, adding noise to each scan since here we treat 
        ...
        for i=1:size(D,1)            
            ...
            distance = sqrt(255/10.^(PSNR/20));
%             noise1 = noise_std(i,1)*randn(size(D(i,:)));
%             noise2 = noise_std(i,1)*randn(size(D(i,:)));
            noise = D(i,mask) + distance*randn(1,length(mask));
            D(i,mask) = D(i,mask) + noise;
