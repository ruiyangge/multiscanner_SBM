function simtb_main_ryg_sMRI(sP)
...
%     end
 if sP.D_noise_FLAG
    for i=1:size(D,1) %%%%%%%%%%%%%%%%%%%%% ryg edit, adding noise to each scan since here we treat 
                       %%%%%%%%%%%%%%%%%%%%% each scan as a structural MRI from one single participant  
        mask = simtb_createmask(sP);
        mask = find(mask == 1);
%         signal_std(i,1) = std(D(i,mask));        
%         noise_std(i,1) = signal_std(i,1)/sP.D_CNR(sub);
        signal_mean(i,1) = mean(D(i,mask));
        noise_std(i,1) = signal_mean(i,1)/sP.D_CNR(sub);
...
%     end
    D_original=D;
    if sP.D_noise_FLAG %%%%%%%%%%%%%%%%%%%%% ryg, adding noise to each scan since here we treat 
                       %%%%%%%%%%%%%%%%%%%%% each scan as a sMRI from one single participant 
        fprintf('\t\tAdding Noise\n')
        for i=1:size(D,1)            
            
            noise1 = noise_std(i,1)*randn(size(D(i,:)));
            noise2 = noise_std(i,1)*randn(size(D(i,:)));
            D(i,:) = sqrt((D(i,:) + noise1).^2 + (noise2).^2);
%             D(i,:) = D(i,:) + noise1;
            clear noise1 noise2 B;
